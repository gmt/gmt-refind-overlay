...
inherit flag-o-matic eutils versionator python-single-r1
...
src_prepare() {
	...
	find BaseTools/{Conf,GNUmakefile,ReadMe.txt,Source,Scripts,BuildEnv} -type f | while read f; do
		edos2unix "${f}"
	done
	[[ $(test-flags-CC -melf_x86_64) == -melf_x86_64 ]] || \
		epatch "${FILESDIR}"/${PN}-${UDK_SLOT}-gcc47_linker_args.patch
}
